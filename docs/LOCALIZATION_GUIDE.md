...
### 1.1 Dual-Tier Translation Pipeline & Transparent Token Shielding
...
```mermaid
flowchart TD
    A["Caller requests string: ougi.text({ msg, lang, stringID, dynamic, raw, values })"] --> B{"Is 'lang' explicitly provided?"}
    B -->|"Yes"| D["Target language = lang"]
    B -->|"No"| C{"Resolve via msg context"}
    C -->|"Message / Interaction / User"| C2["Extract language code via ougi.db().getLang(guildId/userId)"]
    C2 --> D
    C -->|"Default / Undefined"| D2["Target language = 'en'"]
    D2 --> D
    
    D --> E{"Does localization[lang][stringID] exist?"}
    E -->|"Yes (Static Hit)"| F["Retrieve localized template from dictionary"]
    E -->|"No (Static Miss)"| G{"Does fallback localization['en'][stringID] exist?"}
    
    G -->|"Yes"| H["Retrieve English base text"]
    G -->|"No"| I["Treat stringID as raw dynamic text phrase"]
    
    H --> J{"Is target language 'en'?"}
    J -->|"Yes"| K["Base text ready"]
    J -->|"No"| L{"Check Cache: localesCache.db / dynamicLocales.db"}
    
    I --> L
    L -->|"Cache Hit"| M["Retrieve cached translation"]
    L -->|"Cache Miss"| N["Mask tokens {variableName} -> __OGVAR_n__"]
    N --> N2["Fetch translation via Google Translate API"]
    N2 --> N3["Restore tokens __OGVAR_n__ -> {variableName}"]
    N3 --> O["Save translation to dynamicLocales.db"]
    O --> M
    
    F --> P["Apply values interpolation: {var} -> values[var]"]
    K --> P
    M --> P
    P --> Q["Return final rendered string"]
```

### 1.2 Language Resolution Hierarchy
When `ougi.text({ msg, lang, stringID })` is invoked, target language resolution follows strict precedence:
1. **Explicit Language Code (`lang`)**: If an ISO string (e.g., `'en'`, `'es'`, `'fr'`) is provided, it is evaluated directly.
2. **Guild Configuration (`msg.guildId`)**: For Discord messages or interactions originating in a guild, the bot queries `ougi.db().getLang(guildId)`.
3. **User Preference (`msg.author.id` / `interaction.user.id`)**: For direct messages (DMs) or user preferences, user-level settings are evaluated.
...
All localization queries must use the centralized `ougi.text` options object resolver:

```javascript
// Standard asynchronous lookup
const text = await ougi.text({
  msg: msg,            // Discord Message or Interaction object (resolves guild/user language)
  lang: null,          // Or explicit ISO code (e.g., 'en', 'es') if msg is omitted
  stringID: "some_key",// Localization dictionary key or dynamic phrase
  dynamic: false,      // Optional boolean for runtime on-the-fly translation
  raw: false,          // Optional boolean to return raw metadata object { value, fromCode, ... }
  values: {            // Optional key-value map for variable interpolation (keys without {})
    variableName1: "foo",
    variableName2: "bar"
  }
});
...
### 2.2 Placeholder Replacement & Transparent Token Shielding Standard
...
- **Token Syntax in `localization.js`**: `{variableName}` (e.g., `{user}`, `{guild}`, `{amount}`, `{timeStamp}`, `{command}`).
- **Values Object at Call Sites**: Pass values directly as keys in the `values` map (without curly braces). `ougi.text` handles all global interpolation internally:

```javascript
// Localization Dictionary definition (function/localization.js):
"balance_output": "**{user}**'s balance is **{amount} {currency}**."

// Code implementation:
const output = await ougi.text({
  msg,
  stringID: "balance_output",
  values: {
    user: targetUser.username,
    amount: userEco.money,
...
- **Transparent Token Shielding**:
  - Whenever machine translation is executed (dynamic translation or missing static key fallback), placeholder tokens `{variableName}` are shielded as numeric markers (`__OGVAR_0__`, `__OGVAR_1__`) so Google Translate never corrupts or translates token names.
  - Post-translation, tokens are restored and interpolated from `values`.
...
const embed = new Discord.EmbedBuilder()
  .setTitle(await ougi.text({ msg, stringID: "embed_titleKey" }))
  .setDescription(await ougi.text({ msg, stringID: "embed_descKey", values: { user: msg.author.username } }))
  .setColor("#FF008C")
  .addFields({
    name: await ougi.text({ msg, stringID: "embed_fieldNameKey" }),
    value: await ougi.text({ msg, stringID: "embed_fieldValueKey", values: { status: statusText } })
  })
  .setFooter({
    text: await ougi.text({ msg, stringID: "embed_footerKey" }),
...
// - Title ("helpTitle" / "specificHelpTitle")
...
3. **Telemetry & Audit Traceability**:
   - Developer-facing notifications (`davidUserID` DMs and `consoleLogging` channel embeds) use `{ lang: 'en', stringID: "..." }`. Having structured keys for developer audit alerts ensures uniform formatting for moderation reviews, crash tracebacks, and GDPR opt-out logs.
4. **Safety Against Null & Undefined Target**:
   - When calling `ougi.text({ ... })` where `target` might be null or undefined (such as during bot startup before any Discord message is received), pass `{ lang: 'en', stringID: "..." }` or let it default to `'en'`.
...
| **RULE 2** | **Synchronize `localization.js` First** | Always define the key and English translation in `function/localization.js` before referencing `ougi.text({ stringID: "key" })` in any module. |
...
| **RULE 4** | **Strict Options Object & `values` Map** | Always use the object schema `ougi.text({ msg, stringID, values })`. Do not use legacy positional arguments or chained `.replace()` calls on localized strings. |
...
4. Once added, `ougi.text({ msg, stringID: "..." })` will automatically resolve to the new language for any server configured with `ougi lang fr`.
