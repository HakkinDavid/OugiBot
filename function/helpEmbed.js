...
async function (msg) {
  ...
  .addField(await ougi.text(msg, "availableCommands"), await ougi.text(msg, "availableCommandsList") + ": `say`, `answer`, `image`, `translate`, `dice`, `recipe`, `editsnipe`, `survey`, `music`, `skip`, `lyrics`, `curl`, `snipe`, `speak`, `news`, `stats`, `embed`, `tweet`, `learn`, `forget`, `emoji`, `emoji-list`, `language`, `guildlanguage`, `newspaper`, `subscribe`, `unsubscribe`, `prefix`, `blacklist`, `allow`, `setnews`, `setlog`, `info`. " + await ougi.text(msg, "improving"))
  ...
  .addField(await ougi.text(msg, "privacyPolicy") + "\n\u200b", (await ougi.text(msg, "remindLang")).replace(/{c1}/gi, "`ougi language [lang]`").replace(/{c2}/gi, "`ougi help [command]`"));
