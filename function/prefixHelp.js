...
async function (msg) {
  ...
  embed.setDescription(await ougi.text({ msg, stringID: "prefixHelpDesc" }))
  .addFields({name: await ougi.text({ msg, stringID: "specialPermission" }), value: ":warning: " + await ougi.text({ msg, stringID: "onlyOwner" })})
  .addFields({name: await ougi.text({ msg, stringID: "example" }), value: "`ougi prefix " + possiblePrefix[Math.floor(Math.random()*possiblePrefix.length)] + "`"});
