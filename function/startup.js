...
function () {
  if (!settingsOBJ || !localesCache || !dynamicLocales || !knowledgeBase) {
    if (!fs.existsSync(database.settings.file) || !fs.existsSync(database.raffles.file) || !fs.existsSync(database.locales.file) || !fs.existsSync(database.dynamicLocales.file) || !fs.existsSync(database.backup.file)) {
      return false;
    }
