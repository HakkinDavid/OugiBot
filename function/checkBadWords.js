module.exports =
  ...
  async function (msg, replied_to_ougi) {
    let embed = new Discord.EmbedBuilder()
      .setTitle(await ougi.text('en', "log_wordFilterTitle"))
      .setColor("#FF008C")
    ...
    for (i = 0; i < badWord.length; i++) {
      if (msg.content.includes(badWord[i])) {
        let options = [
          await ougi.text(msg, "badwords_resp1"),
          await ougi.text(msg, "badwords_resp2"),
          await ougi.text(msg, "badwords_resp3"),
        ...
        embed.addFields({ name: await ougi.text('en', "log_wordFilterReplied"), value: iSaid });
        ...
        let options = [
          await ougi.text(msg, "badwords_es_resp1"),
          await ougi.text(msg, "badwords_es_resp2"),
          await ougi.text(msg, "badwords_es_resp3"),
          "<:nou:726944701348970496>",
          "<:reverse:726944329754476614>"
        ];
        let iSaid = options[Math.floor(Math.random() * options.length)];
