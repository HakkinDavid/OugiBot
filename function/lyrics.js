...
module.exports = async function (arguments, msg) {
  let lyricsEmbed = new EmbedBuilder()
    ...
    .setColor("#230347")
    .setFooter({ text: await ougi.text({ msg, stringID: "lyrics_footer" }), iconURL: msg.client.user.avatarURL({ dynamic: true, size: 4096 }) });

  let searchQuery = arguments.join(" ");

  if (!searchQuery) {
    if (!msg.guild || !vc[msg.guildId] || !vc[msg.guildId].queue.length) {
      msg.channel.send(await ougi.text({ msg, stringID: "lyrics_nothingPlaying" })).catch(console.error);
      return;
    }
  ...
  try {
    ...
    if (!songHit) {
      msg.channel.send(await ougi.text({ msg, stringID: "lyrics_notAvailable" })).catch(console.error);
      return;
    }
    ...
    if (!lyrics) {
      msg.channel.send(await ougi.text({ msg, stringID: "lyrics_extractFail" })).catch(console.error);
      return;
    }
