...
#### Utility & General
| Command | Syntax / Subcommands | Description | File | Status |
| :--- | :--- | :--- | :--- | :--- |
| **`help`** | `ougi help [command]` | Display the interactive help menu or specific command instructions. | [helpCommand.js](../function/helpCommand.js) | ✅ (Updated with all current commands) |
...
| *(Fallback)* | `ougi <anything else>` | Fallback handler that routes unmatched input to Ougi's generative AI core. | [genAIAbility.js](../function/genAIAbility.js) | ✅ (Updated Pollinations AI integration) |
...
| **`storytell`** | `ougi storytell` | Interactive Storytelling RPG Session with an economic outcome! | [storytellCommand.js](../function/storytellCommand.js) | ✅ (Updated Pollinations AI integration & fallback) |
...
| Command | Syntax / Subcommands | Description | File | Status |
| :--- | :--- | :--- | :--- | :--- |
| **`speak`** | `ougi speak [::lang_code] <text>` | Text-To-Speech (TTS) audio output in voice channel. | [voice.js](../function/voice.js) | ✅ (Fixed voice connection entersState abort handling) |
| **`music`** / **`play`** / **`p`** | `ougi music <query\|URL>` | Play or queue audio in the user's voice channel. | [voiceCallMusic.js](../function/voiceCallMusic.js) | ✅ (Updated error handling and user stream notifications) |
...
| **`lyrics`** | `ougi lyrics <song_title>` | Fetch lyrics for a song via Genius API. | [lyrics.js](../function/lyrics.js) | ✅ (Updated User-Agent headers & DOM selectors) |
...
| **`minesweeper`** | `ougi minesweeper` | Generate an embed Discord minesweeper grid. | [minesweeper.js](../function/minesweeper.js) | ✅ (Added minesweeperHelp documentation) |
| **`snipe`** | `ougi snipe [index]` | Retrieve the last deleted message in the channel. | [shootSniper.js](../function/shootSniper.js) | ✅ (Added bot author check in loadSniper) |
| **`editsnipe`** | `ougi editsnipe [index]` | Retrieve the last edited message in the channel. | [shootSniper.js](../function/shootSniper.js) | ✅ (Added bot author check in loadSniper) |
...
| Root Command | Syntax / Arguments | Description | File | Status |
| :--- | :--- | :--- | :--- | :--- |
| **`#ougi help`** | `#ougi help` | Show root administration help commands. | [helpRootCommand.js](../function/helpRootCommand.js) | ✅ (Updated list of 12 root commands) |
| **`#ougi status`** | `#ougi status ::<dnd\|online\|invisible\|idle> ::<WATCHING\|PLAYING\|STREAMING\|LISTENING\|CUSTOM\|COMPETING> ::<name>` | Change bot presence status, activity type, and activity text. | [statusRootCommand.js](../function/statusRootCommand.js) | ✅ (Updated to Discord.js v14 setPresence & ActivityType) |
| **`#ougi log`** | `#ougi log <emoji\|guilds>` | Generate log files (`allEmoji.txt` / `allGuilds.txt`) and send to storage. | [logRootCommand.js](../function/logRootCommand.js) | ✅ (Updated to AttachmentBuilder & v14 send signature) |
...
| **`#ougi haunt`** | `#ougi haunt <user_id> <message>` | Send a direct message to a user via the bot. | [hauntRootCommand.js](../function/hauntRootCommand.js) | ✅ (Updated to async client.users.fetch) |
| **`#ougi newsletter`** | `#ougi newsletter` | Dispatch global newsletter updates to subscribed channels and users. | [newsletter.js](../function/newsletter.js) | ✅ (Fixed channel fetch assignment & send signature) |
| **`#ougi survey`** | `#ougi survey ::question <q> ::id <id> ::description <d> [::url <u>] [::color <c>]` | Create a global feedback survey popup. | [createSurvey.js](../function/createSurvey.js) | ✅ (Fixed v14 send signature) |
| **`#ougi notifysurvey`** | `#ougi notifysurvey ::id <id> ::description <description>` | Broadcast survey notification to users. | [notifySurvey.js](../function/notifySurvey.js) | ✅ (Updated to async user fetch & fixed send signature) |
...
| **`#ougi raffle-license`** | `#ougi raffle-license <guild_id> [duration_hours] [concurrent_limit] [participant_limit]` | Grant or update server raffle licenses. | [raffleLicenseCommand.js](../function/raffleLicenseCommand.js) | ✅ (New owner command) |
...
| Interaction | Type | Action / Description | File | Status |
| :--- | :--- | :--- | :--- | :--- |
...
| **`ougi_translate_select_lang:<msg_id>`** | String Select Menu | Target language dropdown selector for translations. | [translateCommand.js](../function/translateCommand.js) | ✅ (Removed mx option & dynamically generated options) |
...
| Statement / Trigger | Location | Action | File | Status |
| :--- | :--- | :--- | :--- | :--- |
| `I want to opt out from using Ougi [BOT].` | Direct Messages | Opt out user from Ougi data collection and commands. | [optout.js](../function/optout.js) | ✅ (Fixed pseudoMSG prototype cloning in fan.js & safe access in globalLog.js) |
