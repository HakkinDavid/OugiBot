...
async function (msg) {
  ...
  embed.setDescription(await ougi.text({ msg, stringID: "newsHelpDesc" }))
  .addFields({name: await ougi.text({ msg, stringID: "example" }), value: "`ougi news " + newsTopics[Math.floor(Math.random()*newsTopics.length)] + "`"});
